...
name_group      ::= PATH { "," PATH } | regex ;

characteristic_group ::= time_group | size_group | content_group ;

time_group      ::= newer_group | older_group ;

newer_group     ::= "newer than" group
                  | "newest" [ NUMBER ] ;

older_group     ::= "older than" group
                  | "oldest" [ NUMBER ] ;

size_group      ::= bigger_group | smaller_group ;

bigger_group    ::= "bigger than" ( group | NUMBER )
                  | "biggest" [ NUMBER ] ;

smaller_group   ::= "smaller than" ( group | NUMBER )
                  | "smallest" [ NUMBER ] ;
